...
class AbsoluteGroup:
    ...
    def subgroup(self, generators: List[GroupElement]) -> 'AbsoluteGroup':
        ...
        # Generate subgroup elements with size limit to prevent infinite loops
        ...
        while queue and iterations < max_iterations:
            ...
            if current not in subgroup_elements:
                ...
                # Limit subgroup size for performance
                if len(subgroup_elements) > 100:
                    break
                
                # Add products with existing elements (limited to avoid explosion)
                existing_list = list(subgroup_elements)
                for existing in existing_list[:min(10, len(existing_list))]:
                    new_element1 = self.operate(current, existing)
                    new_element2 = self.operate(existing, current)
                    
                    if new_element1 not in subgroup_elements and len(subgroup_elements) < 100:
                        queue.append(new_element1)
                    if new_element2 not in subgroup_elements and len(subgroup_elements) < 100:
                        queue.append(new_element2)
                
                # Add inverse
                inverse = self.inverse_element(current)
                if inverse not in subgroup_elements and len(subgroup_elements) < 100:
