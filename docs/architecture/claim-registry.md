...
| `CLM-018` | singular division states are representable through `ExtendedRatio`, policy-resolved at runtime, and their core classifier is formalized in Lean | runtime + measured + proved | `implemented` / `measured` / `partial proved` | `docs/api/core/extended-ratio.md`, `docs/benchmarks/claim-closure-results.md`, formal docs | runtime type and API, policy events, tests, baseline artifact, `formal/ACT/ExtendedRatio.lean` | finite, infinite, and indeterminate states are constructible in runtime artifacts, policy handling is benchmarked and test-backed, and the finite/zero/indeterminate division-state classifier compiles in Lean |
